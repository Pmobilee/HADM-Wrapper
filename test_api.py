...
def get_test_image_path(image_name=None):
    ...
    if image_name:
        ...
        for extension in [".jpg", ".jpeg", ".png"]:
            test_image_path = os.path.join(TEST_IMAGES_DIR, f"{image_name}{extension}")
            print(f"Checking image path: {test_image_path}")
...
def test_detection_endpoint(endpoint_path, endpoint_name, test_image_path):
    ...
    try:
        with open(test_image_path, "rb") as f:
            ...
            data = {"confidence_threshold": 0.3, "max_detections": 50}

            print(f"   Using image: {test_image_path}")
            print(f"   Sending request to: {API_BASE_URL}{endpoint_path}")
            start_time = time.time()
            response = requests.post(
                f"{API_BASE_URL}{endpoint_path}", files=files, data=data
            )
            end_time = time.time()
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Image Size: {result.get('image_width')}x{result.get('image_height')}"
                )
                ...
                if local_detections:
                    ...
                    for i, detection in enumerate(local_detections[:3]):  # Show first 3
                        ...
                        print(
                            f"     {i+1}. {detection.get('class_name')} "
                ...
                if global_detection:
                    print(
                        f"   Global Detection: {global_detection.get('class_name')} "
                ...
                print(
                    f"❌ {endpoint_name} detection failed with status {response.status_code}"
                )
                try:
...
def main():
    ...
    parser.add_argument(
        "--image",
        nargs="?",
        help="Name of test image (without .jpg extension) from tests/test_images/ directory",
    )
    parser.add_argument(
        "--list", action="store_true", help="List available test images"
    )
    ...
    results.append(
        (
            "Local Detection",
            test_detection_endpoint("/detect/local", "Local", test_image_path),
        )
    )
    results.append(
        (
            "Global Detection",
            test_detection_endpoint("/detect/global", "Global", test_image_path),
        )
    )
    results.append(
        (
            "Both Detection",
            test_detection_endpoint("/detect/both", "Both", test_image_path),
        )
    )
    results.append(
        (
            "Unified Detection",
            test_detection_endpoint("/detect", "Unified", test_image_path),
        )
    )
